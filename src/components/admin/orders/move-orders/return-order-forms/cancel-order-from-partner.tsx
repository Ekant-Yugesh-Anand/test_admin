...
export default function CancelledReturnFromPartner(props: {
  onClose: () => void;
  ...
  const { isLoading, data } = useQuery(["get-all-reason-retailer-return"], () =>
    shopReason("get", {
      params: "deliverypartner__return",
    })
  );

  const partnerReasonOptions = React.useMemo(() => {
  ...
  const {
    ...
    async onSubmit(values) {
      try {
        setLoading(true);
        const returnReason = partnerReasonOptions.filter(
        ...
        const res = await shopOrdersReturn("post", {
          ...
          data: JSON.stringify({
            ...
            return_order_status: 6,
            return_patner_cancelreason: returnReason[0]?.reason_name,
            return_partner_canceltype:
              returnReason[0]?.type || "Return Retailer",
            return_partner_cancelotherreason:
  ...
  return (
    <Box mt={2}>
      <Typography my={1} variant={"h6"}>
        Cancelled return from delivery partner
      </Typography>
